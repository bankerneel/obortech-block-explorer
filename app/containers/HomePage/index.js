...
import {
  ...
  getTransactionByHourRequest,
  getTransactionByMinuteRequest,
  getCurrentChannelRequest,
  getNetworkListRequest,
  getChannelListRequest,
  getBlockAndTransactionsListRequest,
} from '../../store/actions';
...
const HomePage = ({ history }) => {
  const [selectedTransactionChart, updateSelectedTransactionChart] = useState(
    'TX / Hour',
  );
  const { networkStats } = useSelector(state => state.networkStats);
  const { blockList } = useSelector(state => state.block);
  const { txsByOrg, txsByHour, txsByMinute, blockTxsList } = useSelector(
  ...
  const txsChartData = useMemo(() => {
    let txsData =
      selectedTransactionChart === 'TX / Minute' ? txsByMinute : txsByHour;
  ...
  return (
    <div style={{ marginTop: '20px' }}>
      ...
      >
        <Card sectioned>
          <Card.Header title="Obortech Blockchain Explorer">
