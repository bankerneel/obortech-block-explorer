...
import {
  showLoader,
  hideLoader,
  getCurrentChannelRequestSuccess,
  logoutRequestSuccess,
  getChannelListRequestSuccess,
  // changeChannelRequest,
  changeChannelRequestSuccess,
} from '../../actions';
...
function* changeChannelRequestSaga({ payload }) {
  try {
    ...
    const response = yield axiosMain.get(
      `/channel/changeChannel/${channelGenesis}`,
    );
    if (response.status === 200) {
...
export default function* rootsaga() {
  yield all([
    yield takeEvery(
      actionLabels.GET_CURRENT_CHANNEL_REQUEST,
      getCurrentChannelRequestSaga,
    ),
    yield takeEvery(
      actionLabels.GET_CHANNEL_LIST_REQUEST,
      getChannelListRequestSaga,
    ),
    yield takeEvery(
      actionLabels.CHANGE_CHANNEL_REQUEST,
      changeChannelRequestSaga,
    ),
  ]);
}
